...
复杂或写入任务的需求对齐必须先落为 `route_decision`，再进入实现设计。实现设计必须说明工程深度、large task gate、lane、供料、单文件方案、失败路由和 Git 收尾；具体运行字段见 `rules/11`、`rules/12`、`rules/13`、`rules/14`、`rules/15` 和 `scripts/project_system/project_task_preflight.py`。这不会增加用户的业务确认成本，只是阻止 Codex 在目标、范围或证据不清时猜测执行。
